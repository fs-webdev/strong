...
back.init = function (options, on_done) {
  ...
  this.load(function (err, results) {
    if (err) {
      console.log("Problem loading translations: " + err);
    } else {
      console.log('~~~ done loading translations', results)
    }
  });
};
  ...
  this.eachLoadPath(function (path, namespace) {
    ...
    glob(pattern, { follow: true }, function (err, matches) {
      if (err) {
        ...
        console.log('~~~ loading strong async')
        Promise.all(matches.map(match => self.loadTranslationFile(match, path, namespace)))
...
back.loadTranslationFile = async function (filename, prefix, namespace) {
  try {
    console.log('~~~ load translation file', filename)
    const contents = await readFile(filename)
    console.log('~~~ loaded translation file', filename, contents.length)
